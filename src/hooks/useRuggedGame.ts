...
import { safeCreatePublicKey, isValidSolanaAddress, isValidWallet } from '../utils/walletUtils';
...
export class RuggedGameService {
    ...
    try {
      ...
      if (betToken === 'SOL') {
        ...
        if (!isValidSolanaAddress(RuggedGameService.HOUSE_WALLET_ADDRESS)) {
          throw new Error('Invalid house wallet address configuration');
        }

        // Create PublicKeys safely
        ...
        if (!fromPubkey || !toPubkey) {
          throw new Error('Failed to create valid public keys for transaction');
        }
  ...
  async cashOut(
    ...
    try {
      ...
      // Validate wallet
      if (!isValidWallet(wallet)) {
        throw new Error('Invalid wallet for cashout');
      }
